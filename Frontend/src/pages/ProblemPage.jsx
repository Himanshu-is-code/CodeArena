  ...
  useEffect(() => {
    const fetchProblem = async () => {
      ...
      try {
        ...
        const targetLang = langMap[selectedLanguage]?.toLowerCase();
        const startCodeObj = response.data.startCode?.find(sc =>
          sc.language.toLowerCase() === targetLang || sc.language.toLowerCase() === selectedLanguage.toLowerCase()
        );

        setProblem(response.data);
        setCode(startCodeObj ? startCodeObj.initialCode : (response.data.startCode?.[0]?.initialCode || ''));
        setLoading(false);
      } catch (error) {
        ...
        <div className="flex-1 flex flex-col">
          ...
          {activeRightTab === 'testcase' && (
            <div className="flex-1 p-4 overflow-y-auto">
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3 text-base-content/70">
                  <span className="loading loading-spinner loading-lg"></span>
                  ...
                  <div className="w-full">
                    {runResult.success ? (
                      <div>
                        ...
                        <p className="text-sm mt-2">Runtime: {(runResult.runtime || 0) + " sec"}</p>
                        <p className="text-sm">Memory: {(runResult.memory || 0) + " KB"}</p>
                          ...
                          {runResult.testCases?.map((tc, i) => (
                            <div key={i} className="bg-base-100 p-3 rounded text-xs text-base-content">
                              <div className="font-mono">
                                ...
                                <div className="text-green-600 font-bold mt-1">✓ Passed</div>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-4 space-y-2">
                          {runResult.testCases && runResult.testCases.length > 0 ? (
                            ...
                            <p className="text-xs text-white/80 mt-1">{runResult.error || 'Failed to execute test cases.'}</p>
                          )}
                        </div>
                      </div>
                    )}
          ...
          {activeRightTab === 'result' && (
            <div className="flex-1 p-4 overflow-y-auto">
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3 text-base-content/70">
                  <span className="loading loading-spinner loading-lg"></span>
                  <p className="text-sm">Evaluating submission...</p>
                </div>
              ) : submitResult ? (
                <div className={`alert ${submitResult.accepted ? 'alert-success' : 'alert-error'}`}>
                  <div className="w-full">
                    {submitResult.accepted ? (
                      ...
                      <div>
                        <h4 className="font-bold text-lg">❌ {submitResult.error || 'Submission Rejected'}</h4>
                        <div className="mt-4 space-y-2">
